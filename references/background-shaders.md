...
**Rule for Codex.** Only reach for Tier 2 when:
...
When Codex picks a Tier 2 shader, the shader block nests inside `hero_stage.background` (replacing the old `background_graphics` block that `hero-stage.md` superseded):
