...
**Rule for Codex.** You must pick exactly ONE kit per skill. Do not mix kits in a single preview. If the brand's style doesn't match any kit perfectly, pick the least-wrong one and document the gap in `match_reasoning`.
...
When Codex picks a kit, the skill's `design-model.yaml` gets these fields (not the old flat `library: "..."`):
